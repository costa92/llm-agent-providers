...
| ollama | `ollamaStreamReader` (`ollama/ollama.go:110`) | `Next` (`ollama/ollama.go:120-187`): emits only `EventTextDelta` (`:170`) and `EventDone` (`:161`) | **Tool-call events are not emitted in the stream**. Tool calls are surfaced via the sync `Generate` path only. The Index field is therefore N/A here. ⚠️ **Partial K1 compliance — stream emits no tool-call deltas.** |
...
**Verdict:** K1 holds across openai/anthropic/deepseek/minimax. **Ollama's streaming reader does not emit tool-call delta events** — it only emits text deltas and a terminal `EventDone`. This is the documented gap: callers that need streamed tool-call deltas from Ollama must consult `Generate` for the assembled result, or upgrade `ollama/ollama.go:120-187` to translate the model-specific in-stream tool markers.

The `internal/contract/generate_test.go:103-128` `TestStream_Conformance` suite exercises this for openai/anthropic/ollama; `TestStream_CancelMidStream_Conformance` (`:259-308`) and `TestStream_PartialUsageOnError_Conformance` (`:310-353`) verify the mid-stream cancellation and partial-usage semantics.
