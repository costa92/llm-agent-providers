...
| **ollama** | GREEN | GREEN | K1 GREEN since commit `32f5d59` (2026-05-20): `ollama/ollama.go:211-327` `ingest` emits the full `EventToolCallStart/ArgsDelta/End` triple for both paths — native `tool_calls` field on the done frame (loop at `:244-260`, stable per-call `Index` synthesized in arrival order) and content-parsed `<tool_call>...</tool_call>` markers after Done-flush (`:280-302`). Per-provider tests pin both paths: `TestStream_Ollama_NativeToolCalls` (`ollama/ollama_test.go:609`) and `TestStream_Ollama_QwenXMLContentToolCalls` (`ollama/ollama_test.go:670`). The cross-provider gate `TestStreamToolCalls_Conformance` (`internal/contract/generate_test.go`) enforces K1 across 5/5 providers (6 cases including both ollama paths) using verbatim wire bytes from those tests. K2 is GREEN — Ollama is the K2 reference: `strategyForModel(cfg.model)` and `embeddingDimensionForModel(cfg.model)` make `Capabilities.Tools` and `Capabilities.Embeddings` model-dependent (`ollama/options.go:74-109`). |
...
**Bottom line:** zero RED scores. The K1/K2 invariants hold in source; the remaining YELLOW marks are about (a) deepseek/minimax being a second-class citizen in the `Generate`/cancel/partial-usage portions of the conformance harness, and (b) deepseek/minimax encoding capability as a per-provider constant instead of per-(provider × model). The ollama K1 YELLOW classification was stale (rooted in pre-`32f5d59` code state) and is now lifted — `TestStreamToolCalls_Conformance` exercises 5/5 providers (6 cases) and pins per-call Index stability across Start → ArgsDelta → End plus terminal Done with Usage + FinishReason.
