...
| ollama | `ollama_test.go:139 TestStream_Ollama_Happy` | `ollama_test.go:609 TestStream_Ollama_NativeToolCalls` (native path, asserts `Index 0`) + `ollama_test.go:670 TestStream_Ollama_QwenXMLContentToolCalls` (content-parsed path) + cross-provider `TestStreamToolCalls_Conformance` (both ollama paths) | not covered | not covered | `ollama_test.go:181 TestStream_Ollama_CancelMidStream` + `contract` peer | `contract/generate_test.go:310` (ollama case) |
...
**K1 score:**
- openai — **GREEN**: Index, retry, cancel, partial-usage all tested at the package level and the shared contract suite (including `TestStreamToolCalls_Conformance`).
- anthropic — **GREEN**: Index via tool-block plumbing, no-retry-after-byte at package level, cancel+partial via contract, K1 tool-stream conformance via the shared gate.
- ollama — **GREEN** (lifted from YELLOW, 2026-05-23): stream emits the full typed-union including tool-call deltas via two paths — native `tool_calls` field on done frame (since commit 32f5d59, 2026-05-20) and content-parsed `<tool_call>...</tool_call>` markers (qwen/llama3.1 strategies). Per-call `Index` asserted in both per-provider tests (`TestStream_Ollama_NativeToolCalls`, `TestStream_Ollama_QwenXMLContentToolCalls`) and at the cross-provider gate (`TestStreamToolCalls_Conformance` covers both ollama paths).
- deepseek — **GREEN-via-K1** (formerly YELLOW): Index asserted in `TestStream_DeepSeek_ToolCalls` and now also in the shared `TestStreamToolCalls_Conformance` gate. (Provider still absent from generate/cancel/partial-usage portions of the contract suite — separate concern, see CONCERNS.md "deepseek and minimax bypass the cross-provider conformance suite".)
- minimax — **GREEN-via-K1** (formerly YELLOW): same shape as deepseek — K1 tool-stream now under the shared gate.
...
| Stream tool deltas | `TestStream_OpenAI_ToolCalls` (333) | covered inside `TestStream_Anthropic_PartialJSONFlushesOnContentBlockStop` (196) | `TestStream_Ollama_NativeToolCalls` (609) + `TestStream_Ollama_QwenXMLContentToolCalls` (670) | `TestStream_DeepSeek_ToolCalls` (359) | covered inside `TestStream_MiniMax_PartialJSONFlushesOnContentBlockStop` (220) |
| Stream tool deltas — cross-provider K1 gate | `TestStreamToolCalls_Conformance/openai` | `TestStreamToolCalls_Conformance/anthropic` | `TestStreamToolCalls_Conformance/ollama` × 2 (native + qwen-xml) | `TestStreamToolCalls_Conformance/deepseek` | `TestStreamToolCalls_Conformance/minimax` |
...
2. ~~Ollama stream tool calls are not exercised.~~ **Closed 2026-05-23.** Ollama stream tool events are now exercised by `TestStream_Ollama_NativeToolCalls` (`ollama_test.go:609`), `TestStream_Ollama_QwenXMLContentToolCalls` (`ollama_test.go:670`), and the cross-provider `TestStreamToolCalls_Conformance` gate (both ollama paths). Emission code at `ollama/ollama.go:211-327` has been K1-conformant since commit `32f5d59` (2026-05-20).
