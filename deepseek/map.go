...
func (d *DeepSeek) toSDKRequest(req llm.Request) openai.ChatCompletionNewParams {
	...
	if len(d.tools) > 0 {
		p.Tools = make([]openai.ChatCompletionToolUnionParam, 0, len(d.tools))
...
func (d *DeepSeek) toSDKStreamRequest(req llm.Request) openai.ChatCompletionNewParams {
	p := d.toSDKRequest(req)
...
func (d *DeepSeek) fromSDKResponse(c *openai.ChatCompletion) llm.Response {
	var text string
	if len(c.Choices) > 0 {
