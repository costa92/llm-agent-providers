...
func (d *DeepSeek) effectiveTimeoutForTest() time.Duration { return d.timeout }

func TestNew_HasDefaultTimeout(t *testing.T) {
	m, err := New(WithModel("deepseek-chat"), WithAPIKey("sk-test"))
	if err != nil {
		t.Fatalf("New: %v", err)
	}
...
func TestNew_RespectsExplicitTimeout(t *testing.T) {
	m, err := New(WithModel("deepseek-chat"), WithAPIKey("sk-test"), WithTimeout(10*time.Second))
	if err != nil {
		t.Fatalf("New: %v", err)
	}
