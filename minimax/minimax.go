...
func (m *MiniMax) Generate(ctx context.Context, req llm.Request) (llm.Response, error) {
	sdkReq := m.toSDKRequest(req)
	msg, err := m.client.Messages.New(ctx, sdkReq)
	if err != nil {
		return llm.Response{}, wrapErr(err)
	}
	return m.fromSDKResponse(msg), nil
}
...
func (m *MiniMax) Stream(ctx context.Context, req llm.Request) (llm.StreamReader, error) {
	...
	return &minimaxStreamReader{
		open: func() *ssestream.Stream[sdk.MessageStreamEventUnion] {
			return m.client.Messages.NewStreaming(ctx, sdkReq)
		},
...
func (r *minimaxStreamReader) Next() (llm.StreamEvent, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *minimaxStreamReader) eventToStreamEvents(ev sdk.MessageStreamEventUnion) []llm.StreamEvent {
	...
	case sdk.MessageDeltaEvent:
		r.finish = mapMiniMaxStopReason(string(v.Delta.StopReason))
